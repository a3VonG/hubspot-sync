...
class PlatformClient:
    ...
    def get_organizations_by_ids(self, org_ids: list[str]) -> dict[str, "Organization"]:
        """
        Fetch multiple organizations by ID in a single batch query.
        
        Args:
            org_ids: List of organization UUIDs
            
        Returns:
            Dict mapping org_id -> Organization (with users populated)
        """
        if not org_ids:
            return {}
        ...
        # Fetch users for these organizations
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("""
                SELECT 
                    id,
                    ...
                    created_at
                FROM users
                WHERE organization_id = ANY(%(org_ids)s::uuid[])
                ORDER BY created_at
            """, {"org_ids": org_ids})
            user_rows = cur.fetchall()
        ...
        for row in user_rows:
            ...
            if oid not in users_by_org:
                users_by_org[oid] = []
            users_by_org[oid].append(user)
        
        # Build organization objects
        result: dict[str, Organization] = {}
        for row in org_rows:
            oid = str(row["id"])
            result[oid] = Organization(
                id=oid,
                name=row["name"] or "",
                admin_user_id=str(row["admin_user_id"]) if row.get("admin_user_id") else None,
                paddle_id=row.get("paddle_id"),
                users=users_by_org.get(oid, []),
            )
