...
from analytics.usage_metrics import UsageMetricsComputer
from analytics.order_metrics import OrderMetricsComputer
from analytics.account_metrics import AccountMetricsComputer
from analytics.billing_status import BillingStatusComputer, BillingStatus
...
class PlatformAnalyticsComputer:
    ...
    def compute_for_organization(
        ...
        is_testing = self._determine_testing_status(
            account_metrics, billing_status
        )
    ...
    def compute_for_organizations_batch(
        ...
        for org_id in org_ids:
            ...
            is_testing = self._determine_testing_status(account, billing)
    ...
    def _determine_testing_status(
        ...
        Testing if:
        - No valid Paddle subscription AND no subscription history (fresh signup)
        - OR has NO_BILLING scope
        """
        # Check NO_BILLING scope
        if account_metrics.has_no_billing_scope:
            return True
        
        # Check billing status
        return billing_status.is_testing
